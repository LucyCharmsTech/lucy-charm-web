...
- **Remote images**: IDX/AMPRE primary photos use `https://trreb-image.ampre.ca/...`. That hostname is allowlisted in `next.config.ts` under `images.remotePatterns` so `next/image` can optimize them.
