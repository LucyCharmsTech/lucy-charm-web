...
export default function AdminSellersPage() {
  ...
  const [transactions, setTransactions] = useState<ApiPaginated<SellerTransaction> | null>(null);
  ...
  async function convertLead(leadId: string) {
    ...
    try {
      ...
      setActionMessage('Lead converted to transaction ' + transaction.id.slice(0, 8) + '…');
      await load();
  ...
  if (loading && !leads) {
    return <p className="text-sm text-zinc-500 dark:text-zinc-400">Loading seller pipeline…</p>;
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Assign the right agent, qualify the request, and convert only when the client process is ready.
          </p>
        </div>
        <button
          type="button"
      ...
      {error && (
        <div role="alert" className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300">
          {error}
        </div>
      )}
      {actionMessage && (
        <div role="status" className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-300">
          {actionMessage}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Seller leads</h2>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Unassigned leads remain in the admin queue until you select an agent.
            </p>
          </div>
          <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">{leads?.total ?? 0} total</span>
        </div>
                ...
                {(leads?.items ?? []).map((lead) => (
                  <tr key={lead.id} className="bg-white dark:bg-zinc-950/40">
                    <td className="px-4 py-3 align-top">
                      ...
                      <p className="text-xs text-zinc-500 dark:text-zinc-400">{lead.email || lead.phone || 'No contact method'}</p>
                      <p className="mt-1 text-xs capitalize text-zinc-500 dark:text-zinc-400">{statusLabel(lead.inquiry_type)}</p>
                    </td>
                    ...
                    <td className="px-4 py-3 align-top">
                      <select
                        value={lead.status}
                        disabled={savingId === lead.id || lead.status === 'converted'}
                        onChange={(event) =>
                          saveLead(lead.id, { status: event.target.value as Exclude<SellerLeadStatus, 'converted'> })
                        }
                    ...
                    <td className="px-4 py-3 align-top">
                      <select
                        ...
                        disabled={savingId === lead.id || lead.status === 'converted'}
                        onChange={(event) => saveLead(lead.id, { assigned_agent_id: event.target.value || null })}
                    ...
                    <td className="px-4 py-3 align-top">
                      <button
                        type="button"
                        disabled={Boolean(lead.converted_client_id) || convertingId === lead.id}
                      ...
                      >
                        {convertingId === lead.id ? 'Converting…' : lead.converted_client_id ? 'Converted' : 'Convert'}
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Seller transactions</h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {(transactions?.items ?? []).map((transaction) => (
                  <tr key={transaction.id} className="bg-white dark:bg-zinc-950/40">
                    ...
                    <td className="px-4 py-3 capitalize text-zinc-700 dark:text-zinc-300">
                      {transaction.representation_type ?? 'Not set'} · {transaction.representation_status}
                    </td>
